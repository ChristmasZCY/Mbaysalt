function wrnc_temp(ncid,Lon,Lat,Delement,time,Velement,GA_start_date,varargin)
    ...
    if length(fieldnames(Delement)) == 1  % Delement --> Depth_std || Depth_avg
        if isfield(Delement,'Depth_std')
            ...
            SWITCH.sgm = false;
            SWITCH.avg = false;
            Depth_std = Delement.Depth_std; Temp_std = Velement.Temp_std;
        elseif isfield(Delement,'Depth_avg')
            SWITCH.std = false;
            SWITCH.sgm = false;
            SWITCH.avg = true;
            Depth_avg = Delement.Depth_avg; Temp_avg = Velement.Temp_avg;
        else
    ...
    elseif length(fieldnames(Delement)) == 2  % Delement --> Depth_std Bathy || Depth_avg Bathy
        if isfield(Delement,'Depth_std') && isfield(Delement,'Depth_avg')
            ...
            Temp_std = Velement.Temp_std; Temp_avg = Velement.Temp_avg;
    ...
    elseif length(fieldnames(Delement)) == 3  % Delement --> Depth_std Bathy Siglay || Depth_avg Bathy Siglay
        if isfield(Delement,'Depth_std') && isfield(Delement,'Bathy') && isfield(Delement,'Siglay')
            ...
            Temp_std = Velement.Temp_std; Temp_sgm = Velement.Temp_sgm;
        elseif isfield(Delement,'Depth_avg') && isfield(Delement,'Bathy') && isfield(Delement,'Siglay')
            ...
            Temp_avg = Velement.Temp_avg; Temp_sgm = Velement.Temp_sgm;
        else
            error('The input parameters are wrong!')
        end
    elseif length(fieldnames(Delement)) == 4  % Delement --> Depth_std Bathy Siglay Depth_avg
        if isfield(Delement,'Depth_std') && isfield(Delement,'Bathy') && isfield(Delement,'Siglay') && isfield(Delement,'Depth_avg')
            ...
            Temp_std = Velement.Temp_std; Temp_sgm = Velement.Temp_sgm; Temp_avg = Velement.Temp_avg;
        else
            error('The input parameters are wrong!')
        end
    else
    ...
    if SWITCH.std
        ...
        netcdf.defVarFill(ncid,    temp_std_id,      false,      9.9692100e+36); % 设置缺省值
    ...
    if SWITCH.avg
        ...
        netcdf.defVarFill(ncid,    temp_avg_id,      false,      9.9692100e+36); % 设置缺省值
    ...
    if SWITCH.std
        ...
        netcdf.putAtt(ncid,dep_std_id, 'long_name',    'depth');   % 深度
        netcdf.putAtt(ncid,dep_std_id, 'positive',     'down');    % 深度

        netcdf.putAtt(ncid,temp_std_id, 'units',        'Celsius');                                  % 温度
        netcdf.putAtt(ncid,temp_std_id, 'long_name',    'sea water temperature at standard levels'); % 温度
        netcdf.putAtt(ncid,temp_std_id, 'coordinates',  'standard levels');                          % 温度
    end
    ...
    if SWITCH.sgm
        ...
        netcdf.putAtt(ncid,temp_sgm_id, 'coordinates',  'sigma levels');                          % Temp_sgm
    end
    ...
    if SWITCH.avg
        ...
        netcdf.putAtt(ncid,temp_avg_id, 'units',        'Celsius');                                 % 温度
        netcdf.putAtt(ncid,temp_avg_id, 'long_name',    'sea water temperature at average levels'); % 温度
        netcdf.putAtt(ncid,temp_avg_id, 'coordinates',  'average levels');                          % 温度
    end
