function rtn = wrnc_temp(NC, Lon, Lat, Delement, time, Velement, varargin)
    ...
    %           .Temp_std:   sea temperature at standard levels || required: False  || type: double    || format: matrix
    %           .Temp_sgm:   sea temperature at sigma levels    || required: False  || type: double    || format: matrix
    %           .Temp_avg:   sea temperature at average levels  || required: False  || type: double    || format: matrix
    ...
    % =================================================================================================================
    % Example:
    %       netcdf_fvcom.wrnc_temp(ncid,Lon,Lat,Delement,time,Velement)
    %       netcdf_fvcom.wrnc_temp(ncid,Lon,Lat,Delement,time,Velement,'conf',conf)
    %       netcdf_fvcom.wrnc_temp(ncid,Lon,Lat,Delement,time,Velement,'conf',conf,'INFO')
    %       netcdf_fvcom.wrnc_temp(ncid,Lon,Lat,Delement,time,Velement,'conf',conf,'INFO','Text_len',45)
    %       netcdf_fvcom.wrnc_temp('temp.nc',Lon,Lat,Delement,time,Velement)
    %       netcdf_fvcom.wrnc_temp('temp.nc',Lon,Lat,Delement,time,Velement,'conf',conf)
    %       netcdf_fvcom.wrnc_temp('temp.nc',Lon,Lat,Delement,time,Velement,'conf',conf,'INFO')
    %       netcdf_fvcom.wrnc_temp('temp.nc',Lon,Lat,Delement,time,Velement,'conf',conf,'INFO','Text_len',45)
    %       netcdf_fvcom.wrnc_temp('temp.nc',Lon,Lat,Delement,time,Velement,'dtype','int16')
    % =================================================================================================================
    ...
    switch Method
        case 'LowLevel'
            ...
            if SWITCH.std
                ...
                temp_std_id = netcdf.defVar(ncid, 'temperature_std', 'NC_FLOAT', [londimID, latdimID, depStddimID, timedimID]); % 温度
                netcdf.defVarFill(ncid, temp_std_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
                netcdf.defVarDeflate(ncid, dep_std_id, true, true, 5)
                netcdf.defVarDeflate(ncid, temp_std_id, true, true, 5)
            end
            ...
            if SWITCH.sgm
                ...
                netcdf.defVarFill(ncid, temp_sgm_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
            ...
            if SWITCH.avg
                ...
                temp_avg_id = netcdf.defVar(ncid, 'temperature_avg', 'NC_FLOAT', [londimID, latdimID, depAvgdimID, timedimID]); % 温度
                netcdf.defVarFill(ncid, temp_avg_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
                netcdf.defVarDeflate(ncid, dep_avg_id, true, true, 5)
                netcdf.defVarDeflate(ncid, temp_avg_id, true, true, 5)
            end
            ...
            if SWITCH.std
                ...
                netcdf.putVar(ncid, temp_std_id, [0, 0, 0, 0], [size(Temp_std, 1), size(Temp_std, 2), size(Temp_std, 3), size(Temp_std, 4)], Temp_std); % 温度
            end
            ...
            if SWITCH.avg
                ...
                netcdf.putVar(ncid, temp_avg_id, [0, 0, 0, 0], [size(Temp_avg, 1), size(Temp_avg, 2), size(Temp_avg, 3), size(Temp_avg, 4)], Temp_avg); % 温度
            end
            ...
            if SWITCH.std
                ...
                % ATTRS.temperature_std.key = ATTRS.temperature_std.value; % temperature_std
                for key = fieldnames(ATTRS.temperature_std)'
                    netcdf.putAtt(ncid, temp_std_id, key{1}, ATTRS.temperature_std.(key{1}));
                end % temperature_std
            ...
            if SWITCH.sgm
                ...
                % ATTRS.temperature_sgm.key = ATTRS.temperature_sgm.value; % temperature_sgm
                for key = fieldnames(ATTRS.temperature_sgm)'
                    netcdf.putAtt(ncid, temp_sgm_id, key{1}, ATTRS.temperature_sgm.(key{1}));
                end % temperature_sgm
            ...
            if SWITCH.avg
                ...
                % ATTRS.temperature_avg.key = ATTRS.temperature_avg.value; % temperature_avg
                for key = fieldnames(ATTRS.temperature_avg)'
                    netcdf.putAtt(ncid, temp_avg_id, key{1}, ATTRS.temperature_avg.(key{1}));
                end % temperature_avg
        ...
        case 'HighLevel'
            ...
            if SWITCH.std
                ...
                [scale_factor, add_offset] = calc_scale_offset(Temp_std, dtype);
                nccreate(ncname, 'temperature_std', ...
                ...
                for key = fieldnames(ATTRS.temperature_std)'
                    ncwriteatt(ncname, 'temperature_std', key{1}, ATTRS.temperature_std.(key{1}));
                end

                ncwriteatt(ncname, 'temperature_std', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'temperature_std', 'add_offset', add_offset);
                ncwrite(ncname, 'temperature_std', Temp_std, [1, 1, 1, 1]);
            end
            ...
            if SWITCH.sgm
                ...
                [scale_factor, add_offset] = calc_scale_offset(Temp_sgm, dtype);
                nccreate(ncname, 'temperature_sgm', ...
                ...
                for key = fieldnames(ATTRS.temperature_sgm)'
                    ncwriteatt(ncname, 'temperature_sgm', key{1}, ATTRS.temperature_sgm.(key{1}));
                end

                ncwriteatt(ncname, 'temperature_sgm', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'temperature_sgm', 'add_offset', add_offset);
                ncwrite(ncname, 'temperature_sgm', Temp_sgm, [1, 1, 1, 1]);
            end
            ...
            if SWITCH.avg
                ...
                [scale_factor, add_offset] = calc_scale_offset(Temp_avg, dtype);
                nccreate(ncname, 'temperature_avg', ...
                ...
                for key = fieldnames(ATTRS.temperature_avg)'
                    ncwriteatt(ncname, 'temperature_avg', key{1}, ATTRS.temperature_avg.(key{1}));
                end

                ncwriteatt(ncname, 'temperature_avg', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'temperature_avg', 'add_offset', add_offset);
                ncwrite(ncname, 'temperature_avg', Temp_avg, [1, 1, 1, 1]);
            end
