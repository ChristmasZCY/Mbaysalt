function rtn = wrnc_sand_nemuro(NC, Lon, Lat, Delement, time, Velement, varargin)
    ...
    %       netcdf_fvcom.wrnc_sand_nemuro('sand.nc',Lon,Lat,Delement,time,Velement)
    %       netcdf_fvcom.wrnc_sand_nemuro('sand.nc',Lon,Lat,Delement,time,Velement,'conf',conf)
    %       netcdf_fvcom.wrnc_sand_nemuro('sand.nc',Lon,Lat,Delement,time,Velement,'conf',conf,'INFO')
    %       netcdf_fvcom.wrnc_sand_nemuro('sand.nc',Lon,Lat,Delement,time,Velement,'conf',conf,'INFO','Text_len',45)
    %       netcdf_fvcom.wrnc_sand_nemuro('sand.nc',Lon,Lat,Delement,time,Velement,'dtype','int16')
    % =================================================================================================================
    ...
    switch Method
        case 'LowLevel'
            ...
            if SWITCH.std
                ...
                netcdf.defVarFill(ncid, sand_std_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
            ...
            if SWITCH.sgm
                ...
                netcdf.defVarFill(ncid, sand_sgm_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
            ...
            if SWITCH.avg
                ...
                netcdf.defVarFill(ncid, sand_avg_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
            ...
            if SWITCH.std
                ...
                % ATTRS.sand_std.key = ATTRS.sand_std.value; % sand_std
                for key = fieldnames(ATTRS.sand_std)'
                    netcdf.putAtt(ncid, sand_std_id, key{1}, ATTRS.sand_std.(key{1}));
                end % sand_std
            ...
            if SWITCH.sgm
                ...
                % ATTRS.sand_sgm.key = ATTRS.sand_sgm.value; % sand_sgm
                for key = fieldnames(ATTRS.sand_sgm)'
                    netcdf.putAtt(ncid, sand_sgm_id, key{1}, ATTRS.sand_sgm.(key{1}));
                end % sand_sgm
            ...
            if SWITCH.avg
                ...
                % ATTRS.sand_avg.key = ATTRS.sand_avg.value; % sand_avg
                for key = fieldnames(ATTRS.sand_avg)'
                    netcdf.putAtt(ncid, sand_avg_id, key{1}, ATTRS.sand_avg.(key{1}));
                end % sand_avg
        ...
        case 'HighLevel'
            ...
            if SWITCH.std
                ...
                [scale_factor, add_offset] = calc_scale_offset(Sand_std, dtype);
                nccreate(ncname, 'sand_std', ...
                ...
                for key = fieldnames(ATTRS.sand_std)'
                    ncwriteatt(ncname, 'sand_std', key{1}, ATTRS.sand_std.(key{1}));
                end

                ncwriteatt(ncname, 'sand_std', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'sand_std', 'add_offset', add_offset);
                ncwrite(ncname, 'sand_std', Sand_std, [1, 1, 1, 1]);
            end
            ...
            if SWITCH.sgm
                ...
                [scale_factor, add_offset] = calc_scale_offset(Sand_sgm, dtype);
                nccreate(ncname, 'sand_sgm', ...
                ...
                for key = fieldnames(ATTRS.sand_sgm)'
                    ncwriteatt(ncname, 'sand_sgm', key{1}, ATTRS.sand_sgm.(key{1}));
                end

                ncwriteatt(ncname, 'sand_sgm', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'sand_sgm', 'add_offset', add_offset);
                ncwrite(ncname, 'sand_sgm', Sand_sgm, [1, 1, 1, 1]);
            end
            ...
            if SWITCH.avg
                ...
                [scale_factor, add_offset] = calc_scale_offset(Sand_avg, dtype);
                nccreate(ncname, 'sand_avg', ...
                ...
                for key = fieldnames(ATTRS.sand_avg)'
                    ncwriteatt(ncname, 'sand_avg', key{1}, ATTRS.sand_avg.(key{1}));
                end

                ncwriteatt(ncname, 'sand_avg', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'sand_avg', 'add_offset', add_offset);
                ncwrite(ncname, 'sand_avg', Sand_avg, [1, 1, 1, 1]);
            end
