function wrnc_salt(ncid,Lon,Lat,Delement,time,Velement,GA_start_date,varargin)
    ...
    %           .Salt:       sea salinity at standard levels  || required: True  || type: double || format: matrix
    ...
    if length(fieldnames(Delement)) == 1  % Delement --> Depth_std || Depth_avg
        if isfield(Delement,'Depth_std')
            ...
            SWITCH.sgm = false;
            SWITCH.avg = false;
            Depth_std = Delement.Depth_std; Salt_std = Velement.Salt_std;
        elseif isfield(Delement,'Depth_avg')
            SWITCH.std = false;
            SWITCH.sgm = false;
            SWITCH.avg = true;
            Depth_avg = Delement.Depth_avg; Salt_avg = Velement.Salt_avg;
        else
    ...
    elseif length(fieldnames(Delement)) == 2  % Delement --> Depth_std Bathy || Depth_avg Bathy
        if isfield(Delement,'Depth_std') && isfield(Delement,'Depth_avg')
            ...
            Salt_std = Velement.Salt_std; Salt_avg = Velement.Salt_avg;
    ...
    elseif length(fieldnames(Delement)) == 3  % Delement --> Depth_std Bathy Siglay || Depth_avg Bathy Siglay
        if isfield(Delement,'Depth_std') && isfield(Delement,'Bathy') && isfield(Delement,'Siglay')
            ...
            Salt_std = Velement.Salt_std; Salt_sgm = Velement.Salt_sgm;
        elseif isfield(Delement,'Depth_avg') && isfield(Delement,'Bathy') && isfield(Delement,'Siglay')
            ...
            Salt_avg = Velement.Salt_avg; Salt_sgm = Velement.Salt_sgm;
        else
            error('The input parameters are wrong!')
        end
    elseif length(fieldnames(Delement)) == 4  % Delement --> Depth_std Bathy Siglay Depth_avg
        if isfield(Delement,'Depth_std') && isfield(Delement,'Bathy') && isfield(Delement,'Siglay') && isfield(Delement,'Depth_avg')
            ...
            Salt_std = Velement.Salt_std; Salt_sgm = Velement.Salt_sgm; Salt_avg = Velement.Salt_avg;
        else
            error('The input parameters are wrong!')
        end
    else
    ...
    if SWITCH.std
        ...
        netcdf.defVarFill(ncid,      salt_std_id,      false,      9.9692100e+36);   % 设置缺省值
    ...
    if SWITCH.avg
        ...
        netcdf.defVarFill(ncid,    salt_avg_id,      false,      9.9692100e+36); % 设置缺省值
    ...
    if SWITCH.std
        ...
        netcdf.putAtt(ncid,salt_std_id, 'units',        'psu');                                   % 盐度
        netcdf.putAtt(ncid,salt_std_id, 'long_name',    'sea water salinity at standard levels'); % 盐度
        netcdf.putAtt(ncid,salt_std_id, 'coordinates',  'standard levels');                       % 盐度
    end
    ...
    if SWITCH.sgm
        ...
        netcdf.putAtt(ncid,salt_sgm_id, 'coordinates',  'sigma levels');                       % salt_sgm
    end
    ...
    if SWITCH.avg
        ...
        netcdf.putAtt(ncid,salt_avg_id, 'units',        'psu');                                  % 盐度
        netcdf.putAtt(ncid,salt_avg_id, 'long_name',    'sea water salinity at average levels'); % 盐度
        netcdf.putAtt(ncid,salt_avg_id, 'coordinates',  'average levels');                       % 盐度
    end
