function rtn = wrnc_pco2_ersem(NC, Lon, Lat, Delement, time, Velement, varargin)
    ...
    %           .Pco2_avg:   nitrate nitrogen at average levels || required: False  || type: double    || format: matrix
    ...
    % =================================================================================================================
    % Example:
    %       netcdf_fvcom.wrnc_pco2_ersem(ncid,Lon,Lat,Delement,time,Velement)
    %       netcdf_fvcom.wrnc_pco2_ersem(ncid,Lon,Lat,Delement,time,Velement,'conf',conf)
    %       netcdf_fvcom.wrnc_pco2_ersem(ncid,Lon,Lat,Delement,time,Velement,'conf',conf,'INFO')
    %       netcdf_fvcom.wrnc_pco2_ersem(ncid,Lon,Lat,Delement,time,Velement,'conf',conf,'INFO','Text_len',45)
    %       netcdf_fvcom.wrnc_pco2_ersem('pco2.nc',Lon,Lat,Delement,time,Velement)
    %       netcdf_fvcom.wrnc_pco2_ersem('pco2.nc',Lon,Lat,Delement,time,Velement,'conf',conf)
    %       netcdf_fvcom.wrnc_pco2_ersem('pco2.nc',Lon,Lat,Delement,time,Velement,'conf',conf,'INFO')
    %       netcdf_fvcom.wrnc_pco2_ersem('pco2.nc',Lon,Lat,Delement,time,Velement,'conf',conf,'INFO','Text_len',45)
    %       netcdf_fvcom.wrnc_pco2_ersem('pco2.nc',Lon,Lat,Delement,time,Velement,'dtype','int16')
    % =================================================================================================================
    ...
    switch Method
        case 'LowLevel'
            ...
            if SWITCH.std
                ...
                netcdf.defVarFill(ncid, pco2_std_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
            ...
            if SWITCH.sgm
                ...
                netcdf.defVarFill(ncid, pco2_sgm_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
            ...
            if SWITCH.avg
                ...
                netcdf.defVarFill(ncid, pco2_avg_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
            ...
            if SWITCH.std
                ...
                netcdf.putVar(ncid, pco2_std_id, [0, 0, 0, 0], [size(Pco2_std, 1), size(Pco2_std, 2), size(Pco2_std, 3), size(Pco2_std, 4)], Pco2_std); % pCO2_std
            end
            ...
            if SWITCH.avg
                ...
                netcdf.putVar(ncid, pco2_avg_id, [0, 0, 0, 0], [size(Pco2_avg, 1), size(Pco2_avg, 2), size(Pco2_avg, 3), size(Pco2_avg, 4)], Pco2_avg); % pCO2_avg
            end
            ...
            if SWITCH.std
                ...
                % ATTRS.pco2_std.key = ATTRS.pco2_std.value; % pco2_std
                for key = fieldnames(ATTRS.pco2_std)'
                    netcdf.putAtt(ncid, pco2_std_id, key{1}, ATTRS.pco2_std.(key{1}));
                end % pco2_std
            ...
            if SWITCH.sgm
                ...
                % ATTRS.pco2_sgm.key = ATTRS.pco2_sgm.value; % pco2_sgm
                for key = fieldnames(ATTRS.pco2_sgm)'
                    netcdf.putAtt(ncid, pco2_sgm_id, key{1}, ATTRS.pco2_sgm.(key{1}));
                end % pco2_sgm
            ...
            if SWITCH.avg
                ...
                % ATTRS.pco2_avg.key = ATTRS.pco2_avg.value; % pco2_avg
                for key = fieldnames(ATTRS.pco2_avg)'
                    netcdf.putAtt(ncid, pco2_avg_id, key{1}, ATTRS.pco2_avg.(key{1}));
                end % pco2_avg
        ...
        case 'HighLevel'
            ...
            if SWITCH.std
                ...
                [scale_factor, add_offset] = calc_scale_offset(Pco2_std, dtype);
                nccreate(ncname, 'pCO2_std', ...
                ...
                for key = fieldnames(ATTRS.pco2_std)'
                    ncwriteatt(ncname, 'pCO2_std', key{1}, ATTRS.pco2_std.(key{1}));
                end

                ncwriteatt(ncname, 'pCO2_std', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'pCO2_std', 'add_offset', add_offset);
                ncwrite(ncname, 'pCO2_std', Pco2_std, [1, 1, 1, 1]);
            end
            ...
            if SWITCH.sgm
                ...
                [scale_factor, add_offset] = calc_scale_offset(Pco2_sgm, dtype);
                nccreate(ncname, 'pCO2_sgm', ...
                ...
                for key = fieldnames(ATTRS.pco2_sgm)'
                    ncwriteatt(ncname, 'pCO2_sgm', key{1}, ATTRS.pco2_sgm.(key{1}));
                end

                ncwriteatt(ncname, 'pCO2_sgm', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'pCO2_sgm', 'add_offset', add_offset);
                ncwrite(ncname, 'pCO2_sgm', Pco2_sgm, [1, 1, 1, 1]);
            end
            ...
            if SWITCH.avg
                ...
                [scale_factor, add_offset] = calc_scale_offset(Pco2_avg, dtype);
                nccreate(ncname, 'pCO2_avg', ...
                ...
                for key = fieldnames(ATTRS.pco2_avg)'
                    ncwriteatt(ncname, 'pCO2_avg', key{1}, ATTRS.pco2_avg.(key{1}));
                end

                ncwriteatt(ncname, 'pCO2_avg', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'pCO2_avg', 'add_offset', add_offset);
                ncwrite(ncname, 'pCO2_avg', Pco2_avg, [1, 1, 1, 1]);
            end
