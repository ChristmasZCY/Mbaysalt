    ...
    switch Method
        case 'LowLevel'
            ...
            if SWITCH.aice
                ...
                netcdf.defVarFill(ncid, aice_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
                netcdf.defVarDeflate(ncid, aice_id, true, true, 5)
            end

            if SWITCH.tice
                tice_id = netcdf.defVar(ncid, 'tice', 'NC_FLOAT', [londimID, latdimID, timedimID]); % 海冰厚度
                netcdf.defVarFill(ncid, tice_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
