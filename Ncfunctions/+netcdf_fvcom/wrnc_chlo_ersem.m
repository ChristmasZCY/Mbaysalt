function rtn = wrnc_chlo_ersem(NC,Lon,Lat,Delement,time,Velement,varargin)
    ...
    %       netcdf_fvcom.wrnc_chlo_ersem(ncid,Lon,Lat,Delement,time,Velement,'conf',conf,'INFO')
    %       netcdf_fvcom.wrnc_chlo_ersem(ncid,Lon,Lat,Delement,time,Velement,'conf',conf,'INFO','Text_len',45)
    %       netcdf_fvcom.wrnc_chlo_ersem(ncid,Lon,Lat,Delement,time,Velement,'dtype','int16');
    % =================================================================================================================
    ...
    case 'LowLevel'
        ...
        if SWITCH.std
            ...
            netcdf.defVarFill(ncid, chlo_std_id, false, realmax('single')); % 设置缺省值
        ...
        if SWITCH.sgm
            ...
            netcdf.defVarFill(ncid, chlo_sgm_id, false, realmax('single')); % 设置缺省值
        ...
        if SWITCH.avg
            ...
            netcdf.defVarFill(ncid, chlo_avg_id, false, realmax('single')); % 设置缺省值
        ...
        if SWITCH.std
            ...
            % ATTRS.chlo_std.key = ATTRS.chlo_std.value; % chlorophyll
            for key = fieldnames(ATTRS.chlo_std)'
                netcdf.putAtt(ncid, chlo_std_id, key{1}, ATTRS.chlo_std.(key{1}));
            end                     % chlorophyll
        end
        ...
        if SWITCH.sgm
            ...
            % ATTRS.chlo_sgm.key = ATTRS.chlo_sgm.value; % chlorophyll_sgm
            for key = fieldnames(ATTRS.chlo_sgm)'
                netcdf.putAtt(ncid, chlo_sgm_id, key{1}, ATTRS.chlo_sgm.(key{1}));
            end                     % chlorophyll_sgm
        end
        ...
        if SWITCH.avg
            ...
            % ATTRS.chlo_avg.key = ATTRS.chlo_avg.value; % chlorophyll_avg
            for key = fieldnames(ATTRS.chlo_avg)'
                netcdf.putAtt(ncid, chlo_avg_id, key{1}, ATTRS.chlo_avg.(key{1}));
            end                     % chlorophyll_avg
        end
    ...
    case 'HighLevel'
        ...
        if SWITCH.std
            ...
            [scale_factor, add_offset] = calc_scale_offset(Chlo_std, dtype);
            int8(ncname, 'chlo_std', ...
                ...
                'Shuffle', true);
            for key = fieldnames(ATTRS.chlo_std)'
                ncwriteatt(ncname, 'chlo_std', key{1}, ATTRS.chlo_std.(key{1}));
            end
            ncwriteatt(ncname, 'chlo_std', 'scale_factor', scale_factor);
            ncwriteatt(ncname, 'chlo_std', 'add_offset', add_offset);
            ncwrite(ncname, 'chlo_std', Chlo_std, [1,1,1,1]);
        end
        ...
        if SWITCH.sgm
            ...
            [scale_factor, add_offset] = calc_scale_offset(Chlo_sgm, dtype);
            int8(ncname, 'chlo_sgm', ...
                ...
                'Shuffle', true);
            for key = fieldnames(ATTRS.chlo_sgm)'
                ncwriteatt(ncname, 'chlo_sgm', key{1}, ATTRS.chlo_sgm.(key{1}));
            end
            ncwriteatt(ncname, 'chlo_sgm', 'scale_factor', scale_factor);
            ncwriteatt(ncname, 'chlo_sgm', 'add_offset', add_offset);
            ncwrite(ncname, 'chlo_sgm', Chlo_sgm, [1,1,1,1]);
        end

        if SWITCH.avg
            [scale_factor, add_offset] = calc_scale_offset(Depth_avg, dtype);
            int8(ncname, 'depth_avg', ...
            ...
            [scale_factor, add_offset] = calc_scale_offset(Chlo_avg, dtype);
            int8(ncname, 'chlo_avg', ...
                ...
                'Shuffle', true);
            for key = fieldnames(ATTRS.chlo_avg)'
                ncwriteatt(ncname, 'chlo_avg', key{1}, ATTRS.chlo_avg.(key{1}));
            end
            ncwriteatt(ncname, 'chlo_avg', 'scale_factor', scale_factor);
            ncwriteatt(ncname, 'chlo_avg', 'add_offset', add_offset);
            ncwrite(ncname, 'chlo_avg', Chlo_avg, [1,1,1,1]);
        end
        ...
        ncwriteatt(ncname, varid_GA, 'WriteProgram', ['netcdf_fvcom:', mfilename, ' V', Version]);
