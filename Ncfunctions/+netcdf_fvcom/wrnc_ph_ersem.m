function rtn = wrnc_ph_ersem(NC, Lon, Lat, Delement, time, Velement, varargin)
    %       This function is used to write the pH data to the netcdf file
    ...
    % =================================================================================================================
    % Example:
    %       netcdf_fvcom.wrnc_ph_ersem(ncid,Lon,Lat,Delement,time,Velement)
    %       netcdf_fvcom.wrnc_ph_ersem(ncid,Lon,Lat,Delement,time,Velement,'conf',conf)
    %       netcdf_fvcom.wrnc_ph_ersem(ncid,Lon,Lat,Delement,time,Velement,'conf',conf,'INFO')
    %       netcdf_fvcom.wrnc_ph_ersem(ncid,Lon,Lat,Delement,time,Velement,'conf',conf,'INFO','Text_len',45)
    %       netcdf_fvcom.wrnc_ph_ersem('ph.nc',Lon,Lat,Delement,time,Velement)
    ...
    switch Method
        case 'LowLevel'
            ...
            if SWITCH.std
                ...
                netcdf.defVarFill(ncid, ph_std_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
            ...
            if SWITCH.sgm
                ...
                netcdf.defVarFill(ncid, ph_sgm_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
            ...
            if SWITCH.avg
                ...
                netcdf.defVarFill(ncid, ph_avg_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
            ...
            if SWITCH.std
                ...
                netcdf.putVar(ncid, ph_std_id, [0, 0, 0, 0], [size(Ph_std, 1), size(Ph_std, 2), size(Ph_std, 3), size(Ph_std, 4)], Ph_std); % pH_std
            end
            ...
            if SWITCH.avg
                ...
                netcdf.putVar(ncid, ph_avg_id, [0, 0, 0, 0], [size(Ph_avg, 1), size(Ph_avg, 2), size(Ph_avg, 3), size(Ph_avg, 4)], Ph_avg); % pH_avg
            end
            ...
            if SWITCH.std
                ...
                % ATTRS.ph_std.key = ATTRS.ph_std.value; % ph_std
                for key = fieldnames(ATTRS.ph_std)'
                    netcdf.putAtt(ncid, ph_std_id, key{1}, ATTRS.ph_std.(key{1}));
                end % ph_std
            ...
            if SWITCH.sgm
                ...
                % ATTRS.ph_sgm.key = ATTRS.ph_sgm.value; % ph_sgm
                for key = fieldnames(ATTRS.ph_sgm)'
                    netcdf.putAtt(ncid, ph_sgm_id, key{1}, ATTRS.ph_sgm.(key{1}));
                end % ph_sgm
            ...
            if SWITCH.avg
                ...
                % ATTRS.ph_avg.key = ATTRS.ph_avg.value; % ph_avg
                for key = fieldnames(ATTRS.ph_avg)'
                    netcdf.putAtt(ncid, ph_avg_id, key{1}, ATTRS.ph_avg.(key{1}));
                end % ph_avg
        ...
        case 'HighLevel'
            ...
            if SWITCH.std
                ...
                [scale_factor, add_offset] = calc_scale_offset(Ph_std, dtype);
                nccreate(ncname, 'pH_std', ...
                ...
                for key = fieldnames(ATTRS.ph_std)'
                    ncwriteatt(ncname, 'pH_std', key{1}, ATTRS.ph_std.(key{1}));
                end

                ncwriteatt(ncname, 'pH_std', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'pH_std', 'add_offset', add_offset);
                ncwrite(ncname, 'pH_std', Ph_std, [1, 1, 1, 1]);
            end
            ...
            if SWITCH.sgm
                ...
                [scale_factor, add_offset] = calc_scale_offset(Ph_sgm, dtype);
                nccreate(ncname, 'pH_sgm', ...
                ...
                for key = fieldnames(ATTRS.ph_sgm)'
                    ncwriteatt(ncname, 'pH_sgm', key{1}, ATTRS.ph_sgm.(key{1}));
                end

                ncwriteatt(ncname, 'pH_sgm', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'pH_sgm', 'add_offset', add_offset);
                ncwrite(ncname, 'pH_sgm', Ph_sgm, [1, 1, 1, 1]);
            end
            ...
            if SWITCH.avg
                ...
                [scale_factor, add_offset] = calc_scale_offset(Ph_avg, dtype);
                nccreate(ncname, 'pH_avg', ...
                ...
                for key = fieldnames(ATTRS.ph_avg)'
                    ncwriteatt(ncname, 'pH_avg', key{1}, ATTRS.ph_avg.(key{1}));
                end

                ncwriteatt(ncname, 'pH_avg', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'pH_avg', 'add_offset', add_offset);
                ncwrite(ncname, 'pH_avg', Ph_avg, [1, 1, 1, 1]);
            end
