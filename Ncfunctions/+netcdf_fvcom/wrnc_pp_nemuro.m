function wrnc_pp_nemuro(ncid,Lon,Lat,Delement,time,Velement,varargin)
    ...
    if SWITCH.std
        ...
        netcdf.defVarFill(ncid, pp_std_id, false, 9.9692100e+36);  % 设置缺省值
    ...
    if SWITCH.sgm
        ...
        netcdf.defVarFill(ncid, pp_sgm_id, false, 9.9692100e+36);  % 设置缺省值
    ...
    if SWITCH.avg
        ...
        netcdf.defVarFill(ncid, pp_avg_id, false,  9.9692100e+36); % 设置缺省值
    ...
    if SWITCH.std
        netcdf.putAtt(ncid, dep_std_id, 'units',     'm');     % Pp
        netcdf.putAtt(ncid, dep_std_id, 'long_name', 'depth'); % Pp
        netcdf.putAtt(ncid, dep_std_id, 'positive',  'down');  % Pp

        netcdf.putAtt(ncid, pp_std_id, 'units',       'umolN/L');  % Pp
        netcdf.putAtt(ncid, pp_std_id, 'long_name',   'phytoplankton at standard levels');  % Pp
        netcdf.putAtt(ncid, pp_std_id, 'coordinates', 'standard levels');  % Pp
    end
    ...
    if SWITCH.sgm
        ...
        netcdf.putAtt(ncid, pp_sgm_id, 'units',       'umolN/L');  % Pp_sgm
        netcdf.putAtt(ncid, pp_sgm_id, 'long_name',   'phytoplankton at sigma levels');  % Pp_sgm
        netcdf.putAtt(ncid, pp_sgm_id, 'coordinates', 'sigma levels');  % Pp_sgm
    end

    if SWITCH.avg
        netcdf.putAtt(ncid, dep_avg_id, 'units',     'm');  % Pp
        ...
        netcdf.putAtt(ncid, dep_avg_id, 'positive',  'down');  % Pp
  
        netcdf.putAtt(ncid, pp_avg_id, 'units',       'umolN/L');  % Pp
        netcdf.putAtt(ncid, pp_avg_id, 'long_name',   'phytoplankton at average levels');  % Pp
        netcdf.putAtt(ncid, pp_avg_id, 'coordinates', 'average levels');               % Pp
    end
