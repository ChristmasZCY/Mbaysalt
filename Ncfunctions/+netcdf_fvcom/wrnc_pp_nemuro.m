function rtn = wrnc_pp_nemuro(NC, Lon, Lat, Delement, time, Velement, varargin)
    %       This function is used to write the phytoplankton data to the netcdf file
    ...
    %           .Pp_std:     phytoplankton at standard levels   || required: False  || type: double    || format: matrix
    %           .Pp_sgm:     phytoplankton at sigma levels      || required: False  || type: double    || format: matrix
    %           .Pp_avg:     phytoplankton at average levels    || required: False  || type: double    || format: matrix
    ...
    % =================================================================================================================
    % Example:
    %       netcdf_fvcom.wrnc_pp_nemuro(ncid,Lon,Lat,Delement,time,Velement)
    %       netcdf_fvcom.wrnc_pp_nemuro(ncid,Lon,Lat,Delement,time,Velement,'conf',conf)
    %       netcdf_fvcom.wrnc_pp_nemuro(ncid,Lon,Lat,Delement,time,Velement,'conf',conf,'INFO')
    %       netcdf_fvcom.wrnc_pp_nemuro(ncid,Lon,Lat,Delement,time,Velement,'conf',conf,'INFO','Text_len',45)
    %       netcdf_fvcom.wrnc_pp_nemuro('pp.nc',Lon,Lat,Delement,time,Velement)
    %       netcdf_fvcom.wrnc_pp_nemuro('pp.nc',Lon,Lat,Delement,time,Velement,'conf',conf)
    %       netcdf_fvcom.wrnc_pp_nemuro('pp.nc',Lon,Lat,Delement,time,Velement,'conf',conf,'INFO')
    %       netcdf_fvcom.wrnc_pp_nemuro('pp.nc',Lon,Lat,Delement,time,Velement,'conf',conf,'INFO','Text_len',45)
    %       netcdf_fvcom.wrnc_pp_nemuro('pp.nc',Lon,Lat,Delement,time,Velement,'dtype','int16')
    % =================================================================================================================
    ...
    switch Method
        case 'LowLevel'
            ...
            if SWITCH.std
                ...
                pp_std_id = netcdf.defVar(ncid, 'PP_std', 'NC_FLOAT', [londimID, latdimID, depStddimID, timedimID]); % Pp
                netcdf.defVarFill(ncid, pp_std_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
                netcdf.defVarDeflate(ncid, dep_std_id, true, true, 5)
                netcdf.defVarDeflate(ncid, pp_std_id, true, true, 5)
            end
            ...
            if SWITCH.sgm
                ...
                pp_sgm_id = netcdf.defVar(ncid, 'PP_sgm', 'NC_FLOAT', [londimID, latdimID, sigdimID, timedimID]); % 深度
                ...
                netcdf.defVarFill(ncid, pp_sgm_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值

                netcdf.defVarDeflate(ncid, bathy_id, true, true, 5)
                netcdf.defVarDeflate(ncid, siglay_id, true, true, 5)
                netcdf.defVarDeflate(ncid, pp_sgm_id, true, true, 5)
            end

            if SWITCH.avg
                dep_avg_id = netcdf.defVar(ncid, 'depth_avg', 'NC_FLOAT', [depAvgdimID, twodimID]); % 深度
                pp_avg_id = netcdf.defVar(ncid, 'PP_avg', 'NC_FLOAT', [londimID, latdimID, depAvgdimID, timedimID]); % Pp
                netcdf.defVarFill(ncid, pp_avg_id, false, calc_fillvalue('NC_FLOAT', 'LowLevel')); % 设置缺省值
                netcdf.defVarDeflate(ncid, dep_avg_id, true, true, 5)
                netcdf.defVarDeflate(ncid, pp_avg_id, true, true, 5)
            end
            ...
            if SWITCH.std
                ...
                netcdf.putVar(ncid, pp_std_id, [0, 0, 0, 0], [size(Pp_std, 1), size(Pp_std, 2), size(Pp_std, 3), size(Pp_std, 4)], Pp_std); % Pp_std
            end

            if SWITCH.sgm
                netcdf.putVar(ncid, bathy_id, Bathy); % bathy
                netcdf.putVar(ncid, siglay_id, Siglay); % Siglay
                netcdf.putVar(ncid, pp_sgm_id, [0, 0, 0, 0], [size(Pp_sgm, 1), size(Pp_sgm, 2), size(Pp_sgm, 3), size(Pp_sgm, 4)], Pp_sgm); % Pp_sgm
            end

            if SWITCH.avg
                netcdf.putVar(ncid, dep_avg_id, Depth_avg); % 深度
                netcdf.putVar(ncid, pp_avg_id, [0, 0, 0, 0], [size(Pp_avg, 1), size(Pp_avg, 2), size(Pp_avg, 3), size(Pp_avg, 4)], Pp_avg); % Pp_avg
            end
            ...
            if SWITCH.std
                ...
                % ATTRS.pp_std.key = ATTRS.pp_std.value; % pp_std
                for key = fieldnames(ATTRS.pp_std)'
                    netcdf.putAtt(ncid, pp_std_id, key{1}, ATTRS.pp_std.(key{1}));
                end % pp_std
            ...
            if SWITCH.sgm
                ...
                % ATTRS.pp_sgm.key = ATTRS.pp_sgm.value; % pp_sgm
                for key = fieldnames(ATTRS.pp_sgm)'
                    netcdf.putAtt(ncid, pp_sgm_id, key{1}, ATTRS.pp_sgm.(key{1}));
                end % pp_sgm
            ...
            if SWITCH.avg
                ...
                % ATTRS.pp_avg.key = ATTRS.pp_avg.value; % pp_avg
                for key = fieldnames(ATTRS.pp_avg)'
                    netcdf.putAtt(ncid, pp_avg_id, key{1}, ATTRS.pp_avg.(key{1}));
                end % pp_avg
        ...
        case 'HighLevel'
            ...
            if SWITCH.std
                ...
                [scale_factor, add_offset] = calc_scale_offset(Pp_std, dtype);
                nccreate(ncname, 'PP_std', ...
                ...
                for key = fieldnames(ATTRS.pp_std)'
                    ncwriteatt(ncname, 'PP_std', key{1}, ATTRS.pp_std.(key{1}));
                end

                ncwriteatt(ncname, 'PP_std', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'PP_std', 'add_offset', add_offset);
                ncwrite(ncname, 'PP_std', Pp_std, [1, 1, 1, 1]);
            end
            ...
            if SWITCH.sgm
                ...
                [scale_factor, add_offset] = calc_scale_offset(Pp_sgm, dtype);
                nccreate(ncname, 'PP_sgm', ...
                ...
                for key = fieldnames(ATTRS.pp_sgm)'
                    ncwriteatt(ncname, 'PP_sgm', key{1}, ATTRS.pp_sgm.(key{1}));
                end

                ncwriteatt(ncname, 'PP_sgm', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'PP_sgm', 'add_offset', add_offset);
                ncwrite(ncname, 'PP_sgm', Pp_sgm, [1, 1, 1, 1]);
            end
            ...
            if SWITCH.avg
                ...
                [scale_factor, add_offset] = calc_scale_offset(Pp_avg, dtype);
                nccreate(ncname, 'PP_avg', ...
                ...
                for key = fieldnames(ATTRS.pp_avg)'
                    ncwriteatt(ncname, 'PP_avg', key{1}, ATTRS.pp_avg.(key{1}));
                end

                ncwriteatt(ncname, 'PP_avg', 'scale_factor', scale_factor);
                ncwriteatt(ncname, 'PP_avg', 'add_offset', add_offset);
                ncwrite(ncname, 'PP_avg', Pp_avg, [1, 1, 1, 1]);
            end
            ...
            if ~isempty(conf)
                ...
                for iname = 1:length(fields)
                    ncwriteatt(ncname, '/', fields{iname}, NC.(fields{iname})); % global attribute
                end

            end

            ncwriteatt(ncname, '/', 'product_name', S_name);
            ncwriteatt(ncname, '/', 'WriteProgram', ['netcdf_fvcom:', mfilename, ' V', Version]);
