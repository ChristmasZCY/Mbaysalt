function wrnc_sand(ncid,Lon,Lat,Depth,time,Sand,GA_start_date)
    ...
    %       Sand:            Sand                    || required: True || type: double || format: matrix
    ...
    netcdf.defVarDeflate(ncid, TIME_id, true, true, 5)
    netcdf.defVarDeflate(ncid, sand_id, true, true, 5)
    ...
    netcdf.putVar(ncid,sand_id, [0,0,0,0],[size(Sand,1), size(Sand,2), size(Sand,3),size(Sand,4)],  Sand);     % sand
    ...
    netcdf.putAtt(ncid,sand_id, 'units',        'g/L');                            % sand
    netcdf.putAtt(ncid,sand_id, 'long_name',    'sand');                        % sand
