function wrnc_phytoplankton(ncid,Lon,Lat,Depth,time,Pp,GA_start_date)
    ...
    warning('Mbaysalt:DeprecationWarning: \n%s \n%s \n%s', ...
        ...
        '       See NETCDF_FVCOM.WRNC_PP_NEMURO instead');
    ...
    pp_id  =  netcdf.defVar(ncid,  'PP',       'NC_FLOAT', [londimID, latdimID,depdimID,timedimID]); % phytoplankton

    netcdf.defVarFill(ncid,         pp_id,        false,      realmax('single')); % 设置缺省值
    ...
    netcdf.defVarDeflate(ncid, TIME_id, true, true, 5)
    netcdf.defVarDeflate(ncid, pp_id, true, true, 5)
    ...
    netcdf.putVar(ncid,pp_id,   [0,0,0,0],[size(Pp,1), size(Pp,2), size(Pp,3),size(Pp,4)],  Pp);     % phytoplankton
    ...
    netcdf.putAtt(ncid,pp_id, 'units',        'umolN/L');                            % phytoplankton
    netcdf.putAtt(ncid,pp_id, 'long_name',    'phytoplankton');                        % phytoplankton
