function Postprocess_nemuro(conf_file, interval, yyyymmdd, day_length, varargin)
    ...
    if switch_to_std_level
        osprint2('INFO','Transfor to standard level --> TRUE')
        Depth_std = para_conf.Depth;
    else
        osprint2('INFO','Transfor to standard level --> FALSE')
    end
    ...
    osprint2('INFO',['Date parameter --> ',char(getdate),' total transfor ',num2str(Length),' days'])
    osprint2('INFO',['Method --> ',Method_interpn])
    osprint2('INFO',['Transfor ', interval ,' variable --> pp zp no3 sand'])
