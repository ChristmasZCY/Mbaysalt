...
import os
import time
from christmas.commonCode import split_path, get_date, osprint
...
def read_config(_config_file):
	...
	:return: 配置文件信息
	"""
	Control_dir = read_conf(_config_file)['ControlDir']
	Input_dir = read_conf(_config_file)['ModelOutputDir']
	Standard_dir = read_conf(_config_file)['StandardDir']
	Postprocess_dir = read_conf(_config_file)['PostprocessDir']
	StartDate = int(read_conf(_config_file)['StartDate'])
	DuringDate = int(read_conf(_config_file)['DuringDate'])
	Method_interpn = read_conf(_config_file)['Method_interpn']
	Switch_Cal_hourly = read_conf(_config_file)['Switch_Cal_hourly']
	Switch_Cal_daily = read_conf(_config_file)['Switch_Cal_daily']
