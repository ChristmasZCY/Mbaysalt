...
%
% Updates:
%       2024-11-05: Added Longwave, relative_humidity, air_temperature,  by Christmas;
%
%==========================================================================
...
varlist = {'uwind_speed', 'vwind_speed', 'uwind_stress', 'vwind_stress', ...
           ...
           'SAT', 'SPQ', 'cloud_cover', ...
           'long_wave', 'relative_humidity', 'air_temperature'};
...
% long_wave
if ismember('long_wave', varname)
    long_wave_varid = netcdf.defVar(ncid, 'long_wave', 'float', [node_dimid time_dimid]);
    netcdf.putAtt(ncid, long_wave_varid, 'long_name', 'Long Wave Radiation');
    netcdf.putAtt(ncid, long_wave_varid, 'units', 'W m-2');
end
% relative_humidity
if ismember('relative_humidity', varname)
    relative_humidity_varid = netcdf.defVar(ncid, 'relative_humidity', 'float', [node_dimid time_dimid]);
    netcdf.putAtt(ncid, relative_humidity_varid, 'long_name', 'Relative Humidity');
    netcdf.putAtt(ncid, relative_humidity_varid, 'units', '%');
end
% air_temperature
if ismember('air_temperature', varname)
    air_temperature_varid = netcdf.defVar(ncid, 'air_temperature', 'float', [node_dimid time_dimid]);
    netcdf.putAtt(ncid, air_temperature_varid, 'long_name', 'Air temperature');
    netcdf.putAtt(ncid, air_temperature_varid, 'units', 'degree (C)');
end
