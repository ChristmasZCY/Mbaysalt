...
%
% Updates:
%       2024-12-27: Skipped 't_constituents' not included,  by Christmas;
%
%==========================================================================
...
name = upper(name);
  ...
  for i = 1 : length(name)
      ...
      if isempty(k)
        warning(['Unfound tide constituent name: ' name{i}])
        tidestruc.name(i,1:4) = 'None';
        tidestruc.freq(i,:) = NaN;
        tidestruc.tidecon(i,:) = NaN;
        continue
      end
...
% check
F = find(ismember(tidestruc.name, 'None', 'rows'));
