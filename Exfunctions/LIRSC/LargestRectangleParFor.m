function LRout=LargestRectangleParFor(image,varargin)
...
%EXAMPLES:
%  LRout=LargestRectangle('C:\myDir\myImage.tif')% Run with default values
%  LRout=LargestRectangle('C:\myDir\myImage.tif',0.1,0)% Run with small rotation steps, no iteration
%  LRout=LargestRectangle('C:\myDir\myImage.tif',0,0,0,0)% For axis parallel rectangle
%  Example for rotation step=5, no iteration, starting at 10°, ending at 20°, no graphic:
%     LRout=LargestRectangle('C:\myDir\myImage.tif',5,0,10,20,0)
%LIMITS:
% Images with only 1 px areas will cause an error in LargestRectangleParFor
%REMARK:
...
if iterate 
  ...
  while RotationStep>MinRotationStep 
    ...
    LRtemp=[Limit,0,0,0,0];%area,xmin,xmax,ymin,ymax, angle, sizeImR(1), sizeImR(2),AreaNow
    LRA=zeros(numelAngles,8);%area,xmin,xmax,ymin,ymax,angle, sizeImR(1), sizeImR(2)
    ...
    AN=[AN;LRA(:,6),LRA(:,1)];
    AN=sortrows(AN);
    %area,xmin,xmax,ymin,ymax, angle, sizeImR(1), sizeImR(2)
