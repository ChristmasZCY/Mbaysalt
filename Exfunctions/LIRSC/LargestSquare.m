function LRout=LargestSquare(image,varargin)
...
%EXAMPLES:
% LRout=Largestsquare('C:\myDir\myImage.tif')% Run with default values
% LRout=Largestsquare('C:\myDir\myImage.tif',0.1,0)% Run with small rotation steps, no iteration
% LRout=Largestsquare('C:\myDir\myImage.tif',0,0,0,0)% For axis parallel square
% Example for rotation step=5, no iteration, starting at 10°, ending at 20°, no graphic:
%    LRout=Largestsquare('C:\myDir\myImage.tif',5,0,10,20,0)
%REMARK:
...
if iterate 
  ...
  while RotationStep>MinRotationStep 
    ...
    if RotationStep<0.5 %last number by experiment
      ...
      maxRotIt=min(22,round(6.5+1.7./sqrt(RotationStep)));%9 at 0.4°, 22 at 0.01°
      maxRot=maxRotIt;
        while iR-iL>maxRot
            %delete smaller LR
    ...
    for j=1:numel(angles)
      ...
      if border(1)==0
        ...
        LR=getLR(LR,border,ColLims,RowLims);
        LR(6:8)=[0,sizeImR];
        MinRotationStep=999;% invoces exit of while loop
        break;%Exit for loop
      end
      areaOld=LR(1);
        LR=getLR(LR,border,ColLims,RowLims);
...
function [borderD2,ColLims,RowLims,sizeImR]=GetContour(ImBWc,angle)
  ...
  if isempty(row)
    %totally black image after rotation, only one px areas
    borderD2=0;ColLims=0;RowLims=0;
    return %Abord GetContour
  end
...
function plotResult(ImBW,LR,x,y)
  ...
  figure(hFigPlotLRC);
  cla reset;
  imshow(ImBW,'InitialMagnification','fit','border', 'tight');
  imsize=size(ImBW);
  L2=LR(3)-LR(2)+1;
  L1=LR(5)-LR(4)+1;
  axis on;
